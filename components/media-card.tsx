...
export function MediaCard({ item, type }: { item: MediaItem; type: ContentType }) {
    ...
    <Link href={`/watch/${type}/${item.url_slug}`} className="group block">
      <Card className="overflow-hidden border-0 bg-card py-0 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
        <div className="relative aspect-[2/3] overflow-hidden bg-muted">
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-2 pt-8">
            <div className="flex flex-wrap gap-1">
              ...
              {topCats.map((c) => (
                <Badge key={c} variant="secondary" className="h-5 px-1.5 text-[10px] leading-none bg-white/90 text-black">
                  {c}
                </Badge>
              ))}
            </div>
        ...
        <CardContent className="p-2.5">
          ...
          <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{cats.slice(0, 3).join(" • ")}</p>
        </CardContent>
      </Card>
    </Link>
  );
